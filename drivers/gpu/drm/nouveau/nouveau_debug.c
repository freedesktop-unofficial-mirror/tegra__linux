...
dump_pte(u32 ptepfn, u32 base)
{
	...
	for (k = 0; k < 512; k++) {
		u32 mappfn = pte[0] >> 4;
		bool ptevol = pte[1] & 0x1;
		u32 pteflags = pte[1] & (0x7 << 1);
		if (pte[0] & 1) {
			printk("    %08x -> %08x VOL=%d FLAGS=%x\n", base * 0x4000000 + k * 0x1000, mappfn << 12, ptevol, pteflags);
		}
		pte += 2;
	}
