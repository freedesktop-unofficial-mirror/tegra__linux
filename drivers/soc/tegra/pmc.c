...
static bool tegra_powergate_of_match(struct generic_pm_domain *gpd,
				     const struct of_phandle_args *args)
{
	struct tegra_powergate *powergate = to_powergate(gpd);

	return powergate->id == args->args[0];
}
...
static int tegra_powergate_init(struct tegra_pmc *pmc)
{
	...
	for (i = 0; i < pmc->soc->num_powergates; i++) {
		...
		powergate->base.of_match = tegra_powergate_of_match;
		powergate->base.of_node = pmc->dev->of_node;
